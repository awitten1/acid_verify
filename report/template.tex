...
%%%% 6. ABSTRACT %%%%
\begin{abstract}
In this work we present a database system which provides cryptographically verifiable ACID transactions. We provide a construction for a database that proves to its clients that it provides Serializability. We evaluate our work by comparing the throughput of our verified implementation against an unverified baseline and describe potential improvements to our system.
\end{abstract}
...
(transactions for which the read/write set is not known ahead of time).
...
single-threaded in-memory system as we have implemented it.  Therefore a single core is constantly at $100\%$ while
...
Tree \cite{b11} or the Merkle Patricia Tree \cite{b12}, which is used in Ethereum.
